...
```json
{
  ...
  "permissions": {
    "allow": [
      ...
      "Bash(npm:*)",
      "Bash(yarn:*)",
      "Write(/c/Users/{username}/tfs/*)",
      "Write(/c/Users/{username}/auto/*)",
...
> **本地模式（source_path）额外配置**：当 products.yaml 中配置了 `source_path` 时，worktree 会创建在源仓库父目录的同级（如 `/f/work-space/tenant/mdm-group_1506090/`），该路径不在上述 Write 权限范围内。此时需要额外添加：
> ```json
> "Write(/f/*)"
> ```
> 或者更推荐使用**项目级权限**（见下方第二节），项目级 `project-settings.json` 中已配置无路径限制的 `"Write"` 权限，可覆盖所有本地模式路径。
...
**配置路径**：`~/tfs/{产品名}/.claude/settings.json`
...
| `Bash(git merge:*)` | 合并分支 | ✅ |
| `Write(*)` | 写文件（项目级） | ✅ |
| `Write(/c/Users/{username}/tfs/*)` | 写文件（全局级，限定路径） | ✅ |
...
```
Write(/c/Users/{username}/tfs/*)    # 限定 tfs 目录
Write(/c/Users/{username}/auto/*)   # 限定 auto 目录
...
**配置命令**：
```bash
echo "http://{用户名}:{密码}@{TFS服务器地址}" >> ~/.git-credentials
